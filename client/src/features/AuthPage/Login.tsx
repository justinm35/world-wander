  ...
  return (
    <div className="w-screen flex h-screen justify-center pt-40 over">
      <div className="w-full m-5 xl:w-5/6 h-5/6 bg-white rounded-3xl shadow-md p-5 flex">
          ...
          <img src={AuthBG} className="min-h-full rounded-3xl  object-cover mr-5 w-0 lg:w-1/2 invisible lg:visible"/>
        ...
        <div className="w-full lg:w-1/2 flex flex-col items-center ">
          ...
          <p className="my-3 text-lg font-roboto text-zinc-800">Continue sharing your travels with the world!</p>
            <form className='w-full flex flex-col items-center'>
              <input id="username" type="username" placeholder="Username" value={userData.username} onChange={(e)=>{handleChange(e)}} className=" w-4/6 p-4 mt-10 font-roboto font-semibold text-lg text-zinc-800 border-2 border-gray-500 rounded-lg bg-transparent sm:text-md focus:border-purple-600 focus:shadow-lg "/>
              <input id="password" type="password" placeholder="Password" value={userData.password} onChange={(e)=>{handleChange(e)}} className=" w-4/6 p-4 mt-5 font-roboto font-semibold text-lg text-zinc-800 border-2 border-gray-500 rounded-lg bg-transparent sm:text-md focus:border-purple-600 focus:shadow-lg "/>
              ...
              <button onClick={(e)=>handleSubmit(e)} disabled={isLoading ? true : false} className="w-4/6 bg-zinc-800 text-white rounded-md h-14 text-xl mt-6 active:scale-95 transition disabled:bg-zinc-700 flex items-center justify-center gap-3">Sign In {isLoading && <ThreeDotsBounce color='#ffffff'/>}</button>
