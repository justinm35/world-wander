...
// const displayedIndex = post?.allPosts?.findIndex((post)=> post._id === displayedPost.idCurrent)
  ...
  return (
    <div ref={viewPortRef}
      className='flex flex-col w-full h-full bg-opacity-60 backdrop-blur-sm lg:bg-opacity-100 bg-white px-3 pt-3 snap-always rounded-2xl snap-center relative'>
      ...
      <h1 className="font-roboto text-3xl lg:text-4xl leading-snug tracking-wide font-semibold text-zinc-800 text-center pt-2">{post.destination}</h1>
      {/* <h1 className="font-sans text-3xl font-bold text-slate-200 ">{post.destination}</h1> */}
      <p className="font-roboto text-lg lg:text-xl font-normal text-zinc-600 text-center pt-1">{moment(post.dateTraveled, "YYYY-MM-DD").format('MMMM Do YYYY')}, For {post.tripLength} days</p>
      <h1 className="font-roboto text-lg lg:text-xl font-normal text-zinc-800 mt-4 text-center line-clamp-3">{post.description}</h1>
